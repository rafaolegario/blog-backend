...
const updatePost = async ({ID, title, author, content, picture}) =>{
    ...
    console.log({ID, title, author, content, picture})
...
const postExtended = (post) =>{
   ...
   SaveUpBtn.addEventListener('click', () =>{
    ...
    if(id == codigo){
        updatePost({ID, title: EditInputTitle.value, author:EditInputAuthor.value, content:EditInputContent.value, picture:imageResult})
    }else{
        alert('ID incorreto ou invalido')
    }
